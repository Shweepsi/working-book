...
function timingLines(timings) {
  ...
  const lines = [
    `Attente par page (test) : ${count('exact')} exacte(s), ${count('page')} par n° de page, ${count('fallback')} repli 2 s, ${count('timeout')} plafond, ${count('unchanged') + count('wrapped')} sans changement — moy. ${sec(avg)}, max ${sec(Math.max(...all))}.`,
  ];
